<template>
  <div class="customer-shops-page">
    ...
    <div class="filters-section mb-3 d-flex gap-2 align-items-center">
      ...
      <button class="filter-btn" :class="{ active: filterRating }" @click="toggleFilter('rating')">
        <i class="bi bi-star-fill"></i> Rating
      </button>
      <button class="filter-btn" :class="{ active: filterLocation }" @click="toggleFilter('location')">
        <i class="bi bi-geo-alt-fill"></i> Location
      </button>
      <button class="filter-btn" :class="{ active: filterSort }" @click="toggleFilter('sort')">
    ...
    <div class="row">
      ...
      <div class="col-md-5">
        <div class="map-section sticky-top" style="top: 20px;">
          <h6 class="mb-2"><i class="bi bi-geo-alt-fill"></i> Shop Locations</h6>
    ...
    <div v-if="selectedShop" class="modal-overlay" @click="closeShopProfile">
      <div class="shop-profile-modal" @click.stop>
        ...
        <div class="d-flex gap-3 mb-4">
          <div class="shop-avatar bg-primary text-white rounded-circle d-flex align-items-center justify-content-center" style="width: 60px; height: 60px; font-size: 28px;">
            <i class="bi bi-shop"></i>
          </div>
          <div class="flex-grow-1">
            <h5 class="mb-1">{{ selectedShop.name }}</h5>
            <div class="rating mb-2">
              <span v-for="i in 5" :key="i" :class="i <= selectedShop.rating ? 'text-warning' : 'text-muted'">★</span>
            </div>
            <p class="text-muted small mb-1"><i class="bi bi-geo-alt-fill"></i> {{ selectedShop.address }}</p>
            <p class="text-muted small mb-1"><i class="bi bi-telephone-fill"></i> Contact: {{ selectedShop.contact }}</p>
            <p class="text-muted small"><i class="bi bi-clock-fill"></i> {{ selectedShop.hours }}</p>
          </div>
        </div>
        ...
        <div class="products-section">
          ...
          <div class="row g-3">
            <div v-for="product in selectedShop.products" :key="product.id" class="col-6">
...
const shops = ref([
  {
    ...
    description: 'Premier destination for authentic silk fabrics featuring traditional handloom textiles and modern designer collections. Over 50 years of trusted quality.',
  ...
  },
  {
    ...
    products: [
      { 
        ...
        image: 'https://images.unsplash.com/photo-1636545662955-5225152e33bf?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=400'
      },
      { 
        id: 2, 
        name: 'Eco-Dyed Silk', 
  ...
  },
  {
    ...
    products: [
      { 
        ...
        image: 'https://images.unsplash.com/photo-1636545732552-a94515d1b4c0?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=400'
      },
      { 
        id: 2, 
        name: 'Modern Weaves', 
        ...
        image: 'https://images.unsplash.com/photo-1613132955165-3db1e7526e08?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=400'
      }
    ]
  }
])
